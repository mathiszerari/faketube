<template>
	<div class="form-wrapper">
		<form @submit.prevent="registerFunction" class="form">
			...
			<div class="form-group">
				...
				<input type="password" id="confirmPassword" name="confirmPassword" v-model="confirmPassword" class="input">
			</div>
			<div v-for="(message,index) in formattedMessage?.message" :key="index" class="error-message">
				{{ message }}
			</div>
			<div class="form-group">
				<button type="submit" :disabled="!isFormValid" @click="registerFunction()" class="button">S'inscrire</button>
			</div>
		</form>
	</div>
</template>
  ...
  <script setup>
	...
	const registerFunction = async (key, value) => {
	  ...
	  const {isFetching, error, data: dbmessage} = await useFetch(`http://localhost:8080/register/${pseudo.value}/${email.value}/${password.value}`);
	  message.message = JSON.parse(dbmessage.value)["message"]
  
	  if (JSON.parse(dbmessage.value)["valide"] !== "") {
